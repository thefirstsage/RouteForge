# RouteForge ChatGPT Review File
Generated for reviewing RouteForge code, UI logic, storage, printing, saves, and packaging.
Date: 2026-05-05

## App Summary
- App name: RouteForge
- Type: Windows PySide6 desktop app
- Main UX: Find Businesses -> Build My Day -> Today's Route -> mark outcomes/follow-ups -> print/export
- Current package ZIP: `RouteForge_Distribution/RouteForge-v0.1.2-Windows.zip`
- Current runnable EXE: `RouteForge_Package/RouteForge/RouteForge.exe`

## Recent Context
- Removed the old `Resume Previous Work` button because it froze on large saved-leads files.
- Users should now open named saves through `File > Open Previous Save...`.
- Added named Save / Save As behavior in the File menu.
- Added caching and batched tracker saves to reduce UI freezing.
- Removed embedded map preview as the app now focuses on Google Maps links for navigation.
- Today's Route current stop card was made smaller; Upcoming Stops was made larger.

## Suggested Review Goals
Ask ChatGPT to focus on these areas:
1. Further speed improvements and places where the UI thread may still be blocked.
2. Whether named saves are implemented cleanly and safely.
3. Whether Follow-Ups rendering and saving can be made smoother.
4. Any remaining old branding, confusing UI labels, or dead buttons.
5. Small, targeted edits only. Do not rewrite the whole app.

## File Tree Snapshot
- `assets/README.md`
- `assets/routeforge.ico`
- `build_windows.ps1`
...
- `FEATURES.md`
- `KNOWN_ISSUES.md`
- `Package RouteForge App.cmd`
- `Package RouteForge App.ps1`
- `README.md`
- `Rebuild and Package RouteForge.cmd`
- `requirements.txt`
- `RouteForge.spec`
- `run_instructions.txt`
- `sample-data/sample_leads.csv`
- `saved_presets.json`
- `saved_routes.json`
- `src/desktop_app.py`
- `src/leadgen_tool/__init__.py`
- `src/leadgen_tool/__pycache__/__init__.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/classifier.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/collector.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/config.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/deduper.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/exporter.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/field_reports.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/logging.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/mapping.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/models.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/runner.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/sales_scripts.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/storage.cpython-312.pyc`
- `src/leadgen_tool/__pycache__/ui.cpython-312.pyc`
- `src/leadgen_tool/classifier.py`
- `src/leadgen_tool/cli.py`
- `src/leadgen_tool/collector.py`
- `src/leadgen_tool/config.py`
- `src/leadgen_tool/deduper.py`
- `src/leadgen_tool/exporter.py`
- `src/leadgen_tool/field_reports.py`
- `src/leadgen_tool/logging.py`
- `src/leadgen_tool/mapping.py`
- `src/leadgen_tool/models.py`
- `src/leadgen_tool/runner.py`
- `src/leadgen_tool/sales_scripts.py`
- `src/leadgen_tool/storage.py`
- `src/leadgen_tool/ui.py`
- `src/logs/app.log`
...
﻿{
    ...
    "strict_state_matching":  true,
    "strict_city_matching":  false,
    "include_nearby_cities":  false,
    "exclude_low_value_categories":  true,
    "exclude_low_intent_keywords":  true,
    ...
    "target_leisure_values":  [
                                  "fitness_centre"
                              ],
    "target_office_values":  [
                                 "estate_agent",
                                 "company"
                             ],
    "gas_station_brands":  [
...
### src/desktop_app.py
```python
from leadgen_tool.ui import main
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        leads_tab = QWidget()
        leads_tab_layout = QVBoxLayout(leads_tab)
        leads_tab_layout.setContentsMargins(0, 0, 0, 0)
        leads_tab_layout.addWidget(splitter)
        self.main_tabs.addTab(leads_tab, "Find Leads")

        map_tab = QWidget()
        map_tab_layout = QVBoxLayout(map_tab)
        map_tab_layout.setContentsMargins(0, 0, 0, 0)
        map_controls = QHBoxLayout()
        self.back_to_leads_button = QPushButton("Back to Businesses")
        self.back_to_leads_button.setToolTip("Return to the business list.")
        self.back_to_leads_button.clicked.connect(lambda: self.main_tabs.setCurrentIndex(0))
        ...
        map_controls.addWidget(self.back_to_leads_button)
        ...
        self.main_tabs.addTab(map_tab, "Google Maps")
        ...
        self.route_back_button = QPushButton("Back to Businesses")
        self.route_back_button.setToolTip("Return to the business list.")
        self.route_back_button.clicked.connect(lambda: self.main_tabs.setCurrentIndex(0))
        self.route_back_map_button = QPushButton("Back to Google Maps Route")
        self.route_back_map_button.setToolTip("Return to the Google Maps route overview.")
        self.route_back_map_button.clicked.connect(lambda: self.main_tabs.setCurrentIndex(self.map_tab_index))
        ...
        route_mode_top.addWidget(self.route_back_button)
        route_mode_top.addWidget(self.route_back_map_button)
    ...
    def _load_progress_snapshot(self, snapshot: dict[str, object], fallback_name: str = "") -> None:
        ...
        self.main_tabs.setCurrentIndex(self.route_tab_index if self.route_leads else 0)
    ...
    def _update_action_states(self) -> None:
        ...
        if self.simple_mode:
            ...
            if hasattr(self, "open_google_route_button"):
                self.open_google_route_button.setVisible(has_route)
            self.export_route_button.setVisible(has_route)
            self.print_call_sheet_button.setVisible(has_leads)
            self.start_route_button.setVisible(has_route)
        else:
            self.build_my_day_button.setVisible(True)
            self.route_button.setVisible(True)
            if hasattr(self, "open_google_route_button"):
                self.open_google_route_button.setVisible(True)
            self.export_route_button.setVisible(True)
            self.print_call_sheet_button.setVisible(True)
            self.start_route_button.setVisible(True)
        if hasattr(self, "map_route_button"):
        ...
        self.open_csv_button.setVisible(self.latest_output_path is not None)
        self.export_checked_button.setVisible(has_leads)
    ...
    def _refresh_route_mode(self) -> None:
        ...
        if lead is None:
            ...
            self.current_stop_address.setText("Print the door-knocking sheet or head back to businesses for another route.")
